...
def auto_assembly_loop() -> None:
    boxes = [
        ...
        Box("outputs", 850, 465, 300, 170, "6. Generated Outputs",
            [".step assembly", "sequence PNG/GIF views", "BOM CSV + inventory"], accent=GREEN),
    ...
    boxes = [
        ...
        Box("spec", 1100, 190, 430, 178, "assembly_spec.yaml",
            ["Strict YAML build contract.",
             ...
             "Assembly sequence encodes spacing order.",
        ...
        Box("bom", 585, 465, 430, 178, "BOM JSON + Variant Config",
            ["Public BOM plus variant overrides.",
             ...
             "Spacer thickness stays explicit."], accent=GREEN),
...
def development_evolution() -> None:
    boxes = [
        ...
        Box("today", 650, 535, 300, 190, "Current Build-Out",
            ["assembly spec schema", "component manifest", "gantry-first M3 sequence"], accent=WARN),
        Box("next", 1070, 535, 300, 190, "Next Capability",
            ["connector metadata verification", "BOM parity scoring", "review-view round trip"], accent=GREEN),
...
def capability_stack() -> None:
    boxes = [
        ...
        Box("spec", 925, 180, 340, 145, "Intent Layer",
            ["assembly_spec.yaml", "X -> Y -> Z/frame sequence", "explicit spacers and assumptions"], accent=WARN),
        ...
        Box("outputs", 925, 590, 340, 145, "Builder Output Layer",
            ["configured STEP", "per-step PNG/GIF views", "public BOM and confidence report"], accent=GREEN),
...
def cad_system_monitoring() -> None:
    boxes = [
        Box("cad", 70, 190, 330, 170, "Any CAD System",
            ["desktop CAD", "cloud CAD", "scripted CAD/API"], accent=BLUE),
