...
CADCLAW should remain honest about what it builds. For M3-CRETE, the configurator should place authored STEP components from `[local-path-redacted] and `[local-path-redacted] generate only genuinely parametric stock, then run CADCLAW validation against the emitted assembly.
...
- Public/design BOM source: `[local-path-redacted]
- CAD-derived structural/hardware BOM CSVs under `[local-path-redacted]
