...
- The active M3-2 reference now uses the 125 x 125 x 6 mm C-Beam Gantry Plate XLarge for the two X-to-Y handoff plates and the two-sided X-axis carriage, the smaller 3 mm V-Slot 20-80 gantry plates for Y-to-Z/Z-post carriage interfaces and top-center spreader brackets, the standard Solid V Wheel as the wheel reference, Y gantry C-Beams with the 80 mm dimension vertical, and the authored `ZPMM.step` motor-mount/spacer instead of the generated 6 mm shim.
