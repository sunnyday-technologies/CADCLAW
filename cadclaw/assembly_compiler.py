...
def validate_assembly_spec(
    ...
    """Validate an assembly spec without compiling any geometry.

    Checks the schema (via ``load_assembly_spec``) plus the presence of every
    path the spec points at: reference assets, component manifests, connector
    metadata, and component roots. Declared ``not_built_yet`` items surface as
    warnings, or as failures when ``release`` is set and the item is marked
    ``required_for_release``.

    Shared by ``cadclaw assemble validate-spec`` and the MCP server so both
    report identically.
    """
    ...
    for item in spec.not_built_yet:
        severity = Severity.FAIL if release and item.required_for_release else Severity.WARN
